...
export function EditCustomerPage() {
  ...
  useEffect(() => {
    ...
    try {
      const map = new window.google.maps.Map(mapRef.current, {
        center: { lat: customer.latitude || 31.7683, lng: customer.longitude || 35.2137 },
        zoom: customer.latitude ? 15 : 8,
        minZoom: 7,
        maxZoom: 18,
        restriction: {
      ...
      if (customer.latitude && customer.longitude) {
        updateMarkerOnMap(customer.latitude, customer.longitude);
      }
  ...
  }, [isGoogleMapsReady, customer.latitude, customer.longitude]);
  ...
  async function fetchCustomer(customerId: string) {
    ...
    try {
      ...
      if (data) {
        ...
        if (data.products && Array.isArray(data.products)) {
          const processedProducts = data.products.map((p: any) => {
            ...
            if (matchingProduct) {
              let selectedDimensionIndex = 0;
              const dimensions = getProductDimensions(matchingProduct);

              if (dimensions.length > 1 && p.specifications) {
                const matchingDimIndex = dimensions.findIndex(dim =>
                  dim.length === p.specifications.length &&
                  dim.width === p.specifications.width &&
                  dim.height === p.specifications.height
                );

                if (matchingDimIndex >= 0) {
                  selectedDimensionIndex = matchingDimIndex;
                }
              }
      ...
      alert('שגיאה בטעינת פרטי הלקוח. אנא נסה שוב.');
      navigate('/customers');
    } finally {
  ...
  async function searchAddress(query: string) {
    setCustomer(prev => ({ ...prev, address: query }));
    setAddressError('');
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      alert('שגיאה בעדכון הלקוח. אנא נסה שוב.');
    } finally {
      setIsLoading(false);
    }
  }
